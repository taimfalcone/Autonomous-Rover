...
A three-phase autonomous rover that tracks a black line using IR sensors, detects a target object via Pixy2 color-signature recognition, and retrieves it with a servo-driven claw. All of this is running on a single control loop.wt
...
The system operates in three sequential phases, all managed by a top-level state variable (linebar) that transitions the rover from line tracking to object retrieval.
...
- **Sustained corrections (001, 100):** Turn speed ramps up proportionally to how long the rover has been correcting in one direction. This prevents sluggish responses on sharp curves.  
- **Line lost (000):** The rover sweeps back and forth with increasing width, biased toward the side it last saw the line.
- **Outer sensors and center clear (101):** This means that the rover could be straddling the line either way. The rover then uses 'lastturn,' the direction of the last correction, to pick which way to go instead of guessing randomly.
...
Using a switch on all eight combinations forces you to handle every case separately. The 101 case (both outer sensors lit, center clear) is the kind of thing that's easy to skim over in a long chain of if/else statements, but is obvious when it has its own case block.

**Why ramp turn speed over?**
I tried fixed-speed corrections first and the rover either couldn't keep up with the sharp curves or oscillated on gentle ones. Ramping up the speed based on how long it's been attempting to correct allows the rover to adapt without needing a different constant for every curve radius. This would be inefficient in the code.
